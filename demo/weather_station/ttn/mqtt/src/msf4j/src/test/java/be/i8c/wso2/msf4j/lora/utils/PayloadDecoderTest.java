...
public class PayloadDecoderTest
{
    ...
    public void setup()
    {
        ...
        payloadDecoder = new PayloadDecoder(PAYLOAD_STRING_FORMAT,new UplinkMessageValidator());
    }
    ...
    public void decodePayload_PayloadFormatNotDefined_ShouldThrowPayloadFormatNotDefinedException() throws Throwable
    {
        ...
        when(uplinkMessage.getMetadata()).thenReturn(metadata);
        when(metadata.getTime()).thenReturn(Instant.EPOCH.plusMillis(1).toString());
        payloadDecoder = new PayloadDecoder(null ,new UplinkMessageValidator());

        List<SensorRecord> actual = payloadDecoder.decodePayload(uplinkMessage);
    }

    @Test(expected = PayloadFormatNotDefinedException.class)
    public void decodePayload_PayloadFormatEmpty_ShouldThrowPayloadFormatNotDefinedException() throws Throwable
    {
        when(uplinkMessage.getPayloadRaw()).thenReturn(RAWPAYLOAD_A);
        when(uplinkMessage.getMetadata()).thenReturn(metadata);
        when(metadata.getTime()).thenReturn(Instant.EPOCH.plusMillis(1).toString());
